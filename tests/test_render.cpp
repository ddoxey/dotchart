 ...
 protected:
  static const std::vector<double>& curated_sine() {
    static const std::vector<double> data = {
        0.000000,  0.062791,  0.125333,  0.187381,  0.248690,  0.309017,
        0.368125,  0.425779,  0.481754,  0.535827,  0.587785,  0.637424,
        0.684547,  0.728969,  0.770513,  0.809017,  0.844328,  0.876307,
        0.904827,  0.929776,  0.951057,  0.968583,  0.982287,  1.000000,
        1.000000,  1.000000,  1.000000,  1.000000,  0.982287,  0.968583,
        0.951057,  0.929776,  0.904827,  0.876307,  0.844328,  0.809017,
        0.770513,  0.728969,  0.684547,  0.637424,  0.587785,  0.535827,
        0.481754,  0.425779,  0.368125,  0.309017,  0.248690,  0.187381,
        0.125333,  0.062791,  0.000000,  -0.062791, -0.125333, -0.187381,
        -0.248690, -0.309017, -0.368125, -0.425779, -0.481754, -0.535827,
        -0.587785, -0.637424, -0.684547, -0.728969, -0.770513, -0.809017,
        -0.844328, -0.876307, -0.904827, -0.929776, -0.951057, -0.968583,
        -0.982287, -1.000000, -1.000000, -1.000000, -1.000000, -1.000000,
...
TEST(RenderChart, UnsignedAutoWidthHeight) {
  RecordProperty("objective",
                 "Unsigned render keeps expected row count and cell width.");
  Options opts;
  opts.height = 2;
...
TEST(RenderChart, SignedBaselineBoundary) {
  RecordProperty("objective",
                 "Signed render places baseline as a pixel boundary.");
  Options opts;
  opts.height = 1;
...
TEST_F(RenderChartTest, LabeledCuratedSineOutput) {
  RecordProperty(
      "objective",
      "Curated sine sample renders expected chart with axis labels.");
  Options opts;
  ...
  std::vector<std::string> expected = {
      ...
      from_u8(
          u8"              ⢰ ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰  ⢰   ⢰"),
      from_u8(
          u8"              3 7 13 19 25 31 37 43 49 56 62 68 74 80 86 92 100"),
  };
...
TEST_F(RenderChartTest, LabeledCuratedSinePointOutput) {
  RecordProperty(
      "objective",
      "Curated sine sample renders expected point chart with axis labels.");
  Options opts;
  opts.show_x_axis = true;
  opts.show_y_axis = true;
  opts.y_axis_fmt = "%.6f";
...
TEST(RenderChart, ZeroLineHeightThree) {
  ...
  std::vector<std::string> expected = {
      from_u8(u8"1.0 ⠉⠉ ⠀"), from_u8(u8"0.5 ⠤⠤ ⠀"), from_u8(u8"0.0 ⠤⠤ ⠤")};
...
TEST(RenderChart, ZeroLineHeightThreeAsymmetric) {
  RecordProperty("objective",
                 "Zero line alignment with asymmetric range at height=3.");
  Options opts;
  opts.height = 3;
  opts.show_y_axis = true;
  opts.y_axis_fmt = "%.1f";
  ...
  std::vector<std::string> expected = {from_u8(u8" 6.0 ⠉⠉ ⠀⡇"),
                                       from_u8(u8"     ⠤⠤⠤⠤⢧"),
                                       from_u8(u8"-5.0 ⣀⣀ ⠀⢸")};
