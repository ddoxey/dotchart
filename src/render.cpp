...
static int detect_max_colors() {
  ...
  if (env_contains(term, "256color") || env_contains(colorterm, "256color") ||
      env_contains(colorterm, "truecolor") ||
...
static RenderColorMode detect_color_mode(Options::ColorMode mode,
                                         int max_colors) {
...
static bool fmt_expects_int(const std::string& fmt) {
  ...
  for (size_t i = 0; i < fmt.size(); ++i) {
    ...
    size_t j = i + 1;
    while (j < fmt.size() &&
           std::string("+- #0").find(fmt[j]) != std::string::npos)
      ++j;
...
static void emit_debug_table(const Options& opts, const TermSize& ts,
                             int out_cols_cap, int usable_cols, int cells_w,
                             int height_rows, int target_samples,
                             int input_points, int resampled_points,
                             double scale_max, const DataStats& st,
                             RenderColorMode color_mode, int max_colors) {
  ...
  const char* cm =
      (color_mode == RenderColorMode::Ansi256)
          ? "256"
...
static int determine_target_columns(const Options& opts, int term_cols) {
  ...
  return term_cols;  // just return terminal width cap; actual default width is
                     // data-driven.
}

static std::vector<double> resample_maxbin(const std::vector<double>& in,
                                           int target) {
  if (target <= 0) return {};
  ...
  for (int j = 0; j < target; ++j) {
    int start =
        static_cast<int>(std::floor((static_cast<double>(j) * n) / target));
    int end =
        static_cast<int>(std::floor((static_cast<double>(j + 1) * n) / target));
...
static std::vector<double> resample_extreme_abs(const std::vector<double>& in,
                                                int target) {
  if (target <= 0) return {};
  if (in.empty()) return std::vector<double>(static_cast<size_t>(target), 0.0);
  if (static_cast<int>(in.size()) == target) return in;
...
std::vector<std::string> render_chart(const Options& opts,
  ...
  if (opts.show_y_axis) {
    ...
    y_axis_cols =
        static_cast<int>(max_label) + 1 + tick_len;  // label + space + tick
    ...
    const std::string bottom_tick =
        utf8_encode(0x28C0);  // dots 7 and 8 (row 4)
    for (int row = 0; row < H; ++row) {
  ...
  if (signed_mode &&
      (opts.width.kind != WidthSpec::Kind::Auto || n > target_samples)) {
  ...
  if (opts.debug) {
    emit_debug_table(opts, ts, out_cols_cap, usable_cols, cells_w, H,
                     target_samples, static_cast<int>(values.size()),
                     static_cast<int>(samples.size()),
                     signed_mode ? scale_max : M, st, color_mode, max_colors);
    if (signed_mode) {
      const int P = H * 4;
  ...
  // We always render up to target_samples, but if auto+no-resample and samples
  // shorter, render only those samples (the width will already be tight).
